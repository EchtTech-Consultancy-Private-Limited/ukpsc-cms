...
class Admin_roles extends MY_Controller
{
    ...
    function check_admin_role($id = 0)
    {
        ...
        $this->db->where(
            "admin_role_title",
            $this->input->post("admin_role_title")
        );
    ...
    public function module_edit($id = 0)
    {
        ...
        if ($this->input->post("submit")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                redirect(
                    base_url("admin/admin_roles/module_edit/" . $id),
                    "refresh"
                );
            } else {
                $data = [
    ...
    public function sub_module($module_id = null)
    {
        ...
        $data["records"] = $this->admin_roles->get_sub_module_by_module(
            $module_id
        );
    ...
    public function sub_module_add()
    {
        ...
        if ($this->input->post("submit")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                redirect(
                    base_url("admin/admin_roles/sub_module_add/" . $parent),
                    "refresh"
                );
            } else {
                $data = [
                ...
                if ($result) {
                    $this->session->set_flashdata(
                        "success",
                        "Sub Module has been added successfully!"
                    );

                    redirect(
                        base_url("admin/admin_roles/sub_module/" . $parent)
                    );
                }
            }
        } else {
    ...
    public function sub_module_edit($id = 0)
    {
        ...
        if ($this->input->post("submit")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                redirect(
                    base_url("admin/admin_roles/sub_module_edit/" . $id),
                    "refresh"
                );
            } else {
                ...
                if ($result) {
                    $this->session->set_flashdata(
                        "success",
                        "sub module has been Updated successfully!"
                    );
    ...
    public function sub_module_delete($id = 0, $parent = 0)
    {
        ...
        $this->session->set_flashdata(
            "msg",
            "Sub Menu has been Deleted Successfully."
        );
