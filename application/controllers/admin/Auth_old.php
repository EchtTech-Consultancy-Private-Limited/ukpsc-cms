...
class Auth extends MY_Controller
{
    ...
    public function checkmail()
    {
        ...
        $mail_data = [
            ...
            "verification_link" =>
                base_url("admin/auth/verify/") . "/" . md5(rand(0, 1000)),
        ];
        ...
        if ($email) {
            $this->session->set_flashdata(
                "Success",
                "Your account has been successfully created, please verify it by clicking the activation link that has been send to your email address"
            );
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($result) {
                    ...
                    if ($result["is_active"] == 0) {
                        $this->session->set_flashdata(
                            "error",
                            "Account is disabled as per as your request !"
                        );
                    ...
                    if ($result["is_admin"] == 1) {
                        ...
                        $admin_role_id =
                            $this->session->userdata["admin_role_id"];
                        ...
                        if ($admin_role_id == 6) {
                            ...
                            if ($match != 0) {
                                ...
                                redirect(
                                    base_url(
                                        "admin/consent_letter/consent_add"
                                    ),
                                    "refresh"
                                );
                            }
                        } elseif ($admin_role_id == 5) {
                            // redirect(base_url('admin/dashboard'), 'refresh');

                            redirect(
                                base_url("admin/consent_letter/consent_list"),
                                "refresh"
                            );
                        } else {
                            redirect(base_url("admin/dashboard"), "refresh");
    ...
    public function register()
    {
        if ($this->input->post("submit")) {
            ...
            $this->form_validation->set_rules(
                "email",
                "Email",
                "trim|is_unique[ci_admin.email]|required"
            );
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($result) {
                    ...
                    if ($email) {
                        $this->session->set_flashdata(
                            "success",
                            "Your account has been successfully created, please verify it by clicking the activation link that has been send to your email address and generate your new password to log in to UKPSC."
                        );
    ...
    public function verify()
    {
        if ($this->input->post("submit")) {
            ...
            if ($verified) {
                ...
                $this->auth_model->reset_password1(
                    $verified,
                    $new_password,
                    $new_password1
                );
                ...
                $this->session->set_flashdata(
                    "success",
                    "New password has been Updated successfully.Please login below with latest Login id and password"
                );
    ...
    public function forgot_password()
    {
        if ($this->input->post("submit")) {
            ...
            if ($response) {
                ...
                $this->auth_model->update_reset_code(
                    $pwd_reset_code,
                    $response["admin_id"]
                );
                ...
                $mail_data = [
                    ...
                    "reset_link" => base_url(
                        "admin/auth/reset_password/" . $pwd_reset_code
                    ),
                ];
                ...
                if ($email) {
                    $this->session->set_flashdata(
                        "success",
                        "We have sent instructions for resetting your password to your email"
                    );
                    ...
                    $this->session->set_flashdata(
                        "error",
                        "There is the problem on your email"
                    );
    ...
    public function reset_password($id = 0)
    {
        ...
        if ($this->input->post("submit")) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $this->session->set_flashdata(
                    "success",
                    "New password has been Updated successfully.Please login below"
                );
            ...
            if ($result) {
                ...
                $this->session->set_flashdata(
                    "error",
                    "Password Reset Code is either invalid or expired."
                );
    ...
    public function get_country_states()
    {
        ...
        $options =
            ["" => "Select Option"] + array_column($states, "name", "id");
    ...
    public function get_state_cities()
    {
        $cities = $this->db
            ->select("*")
            ->where("state_id", $this->input->post("state"))
            ->get("ci_cities")
            ->result_array();

        $options =
            ["" => "Select Option"] + array_column($cities, "name", "id");
