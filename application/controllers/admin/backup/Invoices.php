...
class Invoices extends MY_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->input->post("submit")) {
            ...
            if ($company_id) {
                $items_detail = [
                    "product_description" => $this->input->post(
                        "product_description"
                    ),
                ...
                $data["invoice_data"] = [
                    ...
                    "created_date" => date(
                        "Y-m-d",
                        strtotime($this->input->post("billing_date"))
                    ),
                ];

                $invoice_data = $this->security->xss_clean(
                    $data["invoice_data"]
                );
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->input->post("submit")) {
            ...
            $company_id = $this->invoice_model->update_company(
                $data,
            ...
            $data["invoice_detail"] = $this->invoice_model->get_invoice_by_id(
                $id
            );
    ...
    public function create_pdf($id = 0)
    {
        ...
        $stylesheet = file_get_contents(
            base_url("public/dist/css/mpdfstyletables.css")
        );
    ...
    public function delete($id)
    {
        ...
        $result = $this->db->delete("ci_payments", ["id" => $id]);
