...
class General_settings extends MY_Controller
{
    ...
    public function index()
    {
        $data[
            "general_settings"
        ] = $this->setting_model->get_general_settings();
    ...
    public function add()
    {
        ...
        $data = [
            ...
            "recaptcha_secret_key" => $this->input->post(
        ...
        if (!empty($_FILES["logo"]["name"])) {
            ...
            $result = $this->functions->file_insert(
                $path,
                "logo",
                "image",
                "9097152"
    ...
    public function email_templates()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                $result = $this->setting_model->update_email_template(
                    $data,
                    $id
                );
            ...
            $this->load->view(
                "admin/general_settings/email_templates/templates_list",
                $data
            );
    ...
    public function get_email_template_content_by_id()
    {
        ...
        $data[
            "template"
        ] = $this->setting_model->get_email_template_content_by_id($id);

        $variables = $this->setting_model->get_email_template_variables_by_id(
            $id
        );

        $data["variables"] = implode(
            ",",
            array_column($variables, "variable_name")
        );
    ...
    public function email_preview()
    {
        if ($this->input->post("content")) {
            ...
            echo $this->load->view(
                "admin/general_settings/email_templates/email_preview",
                $data,
                true
            );
        }
